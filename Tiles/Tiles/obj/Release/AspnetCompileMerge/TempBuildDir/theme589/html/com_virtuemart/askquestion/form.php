	...
	if(empty($this->login) or VmConfig::get('recommend_unauth',false)){
		...
		<div class="ask-a-question-view">
			<div class="page_heading">
				<h1 class="page_title"><?php echo JText::_('COM_VIRTUEMART_PRODUCT_ASK_QUESTION')  ?></h1>
			</div>
			...
			<div class="product_summary">
					...
					<h2 class="product_title"><?php echo $this->product->product_name ?></h2>
			...
			<div class="form-field">
				...
				<form method="post" class="form-validate" action="<?php echo JRoute::_('index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id='.$this->product->virtuemart_product_id.'&virtuemart_category_id='.$this->product->virtuemart_category_id.'&tmpl=component', FALSE) ; ?>" name="askform" id="askform">
					...
					<div class="form-group">
						<label><?php echo JText::sprintf('COM_VIRTUEMART_ASK_COMMENT', $min, $max); ?></label>
						<textarea class="validate[required,minSize[<?php echo $min ?>],maxSize[<?php echo $max ?>]] field" id="comment" name="comment" rows="4"><?php echo $askQuestionData['comment'] ?></textarea>
					</div>

					<button value="<?php echo JText::_('COM_VIRTUEMART_ASK_SUBMIT')  ?>" type="submit" name="submit_ask" class="highlight-button btn btn-default"><span><span><?php echo JText::_('COM_VIRTUEMART_ASK_SUBMIT')  ?></span></span></button>
					
					<div class="form-inline">		
                    <br/>				
						<?php echo JText::_('COM_VIRTUEMART_ASK_COUNT')  ?>
						<input type="text" value="0" size="4" class="counter inputbox" id="counter" name="counter" maxlength="4" readonly="readonly" />
					</div>
