...
?>
<div class="product_price" id="productPrice<?php echo $this->product->virtuemart_product_id ?>">
	<?php
	...
	if ($this->product->prices['salesPrice']<=0 and VmConfig::get ('askprice', 1) and isset($this->product->images[0]) and !$this->product->images[0]->file_is_downloadable) {
		...
		?>
		<a class="ask-a-question bold" href="<?php echo $askquestion_url ?>" rel="nofollow" ><?php echo JText::_ ('COM_VIRTUEMART_PRODUCT_ASKPRICE') ?></a>
		<?php
	} else {
	...
	if (abs($this->product->prices['discountAmount']) > 0 && ($this->product->prices['salesPrice'] < $this->product->prices['salesPriceWithDiscount'] ) ){
		echo '<span class="price-crossed" >' .$this->currency->createPriceDiv ('salesPriceWithDiscount', 'COM_VIRTUEMART_PRODUCT_SALESPRICE_WITH_DISCOUNT', $this->product->prices). "</span>";
	}
